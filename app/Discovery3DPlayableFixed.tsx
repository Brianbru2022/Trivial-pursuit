...
export default function Discovery3DPlayableFixed(){
 ...
 function resolve(id:string){const loc=BY_ID[id];setPlayers(ps=>ps.map((p,i)=>{if(i!==active)return p;let n={...p};if(loc.reward)n={...n,[loc.reward]:n[loc.reward]+2};if(loc.kind==="volcano")n=Math.random()<.5?{...n,glory:n.glory+2}:{...n,influence:Math.max(0,n.influence-1)};if(loc.kind==="port"){const keys:Resource[]=["influence","knowledge","relics"];const poorest=[...keys].sort((a,b)=>n[a]-n[b])[0];n={...n,[poorest]:n[poorest]+1}}return n}));if(loc.kind==="temple"||loc.kind==="city")window.setTimeout(()=>drawDiscovery(),50);else if(loc.kind==="volcano")setCard("Volcanic Gamble resolved — gain 2 Glory or lose 1 Influence.");else if(loc.kind==="monument"&&canEnterMonument(player)){setWinner(player.name);setCard(`${player.name} reaches the Great Monument!`)}else setCard(loc.reward?`${loc.name}: gain 2 ${loc.reward}.`:`${loc.name}: expedition action resolved.`);setMessage("Resolve the destination, then end your turn.")}
 ...
 function endTurn(){if(moving)return;const n=(active+1)%players.length;setActive(n);setRoll(null);setReachable([]);setCard(null);setSelected(players[n].location);setMessage(`${players[n].name}: roll the die.`)}
 return <main className={styles.page}>
  <header className={styles.header}><div><small>STABLE FULL-BOARD VIEW</small><h1>DISCOVERY LEGENDS</h1></div><div className={styles.status}>{players.map((p,i)=><button key={p.colour} onClick={()=>{if(!moving){setActive(i);setSelected(p.location)}}} className={i===active?styles.active:""}><i style={{background:COLOUR[p.colour]}}/>{p.name}<span>★ {p.glory}</span></button>)}</div></header>
  <section className={styles.boardShell}>
   <Canvas orthographic shadows dpr={[1,1.45]} camera={{position:[18,16,18],zoom:34,near:.1,far:140}}>
    <SafeCamera/><fog attach="fog" args={["#102934",32,58]}/><color attach="background" args={["#102934"]}/><ambientLight intensity={.74}/><hemisphereLight args={["#dceeff","#51412c",1.35]}/><directionalLight castShadow position={[-10,17,11]} intensity={4.0} shadow-mapSize-width={2048} shadow-mapSize-height={2048}/><directionalLight position={[10,8,-8]} intensity={.75} color="#ffd79c"/>
    ...
    <ContactShadows position={[0,.02,0]} opacity={.44} scale={28} blur={2.8} far={12}/>
   </Canvas>
...
function SafeCamera(){const {camera}=useThree();useEffect(()=>{camera.position.set(18,16,18);camera.lookAt(0,.2,0);const ortho=camera as THREE.OrthographicCamera;ortho.zoom=34;ortho.updateProjectionMatrix()},[camera]);return null}
